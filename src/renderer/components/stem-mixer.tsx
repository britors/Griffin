import { STEM_LABELS, type OutputRoute, type StemName } from '../../shared/types'
...
export function StemMixer() {
  const { volumes, pans, routes, muted, solo, setVolume, setPan, setRoute, toggleMute, toggleSolo } = usePlayer()
  return <section className="panel mixer"><div className="section-heading"><div><span className="eyebrow">MIXER</span><h2>Seus stems</h2></div><span className="badge">4 canais</span></div><p className="mixer-help">Estéreo usa o dispositivo padrão. Canal L/R fixa o stem no canal correspondente e mantém o sincronismo.</p><div className="stem-grid">
    {stems.map((stem) => <div className={`stem ${solo === stem ? 'is-solo' : ''}`} key={stem}>
      ...
      <label className="route-control"><span>SAÍDA</span><select aria-label={`Saída de ${STEM_LABELS[stem]}`} value={routes[stem]} onChange={(event) => setRoute(stem, event.target.value as OutputRoute)}><option value="stereo">Estéreo</option><option value="left">Canal L</option><option value="right">Canal R</option></select></label>
