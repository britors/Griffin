...
export function registerSeparationHandlers(service: SeparationApplicationService, getWebContents: () => WebContents | undefined) {
  ...
  ipcMain.handle('separation:start', (_event, track: Track, target?: StemName) => service.start(track, (progress) => getWebContents()?.send('separation:progress', progress), target))
