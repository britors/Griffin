...
export class AudioExportApplicationService {
  ...
  async export(trackId: string, options: AudioExportOptions): Promise<AudioExportResult> {
    ...
    const rendered = await this.processor.render(stems, options)
    ...
    return { path, duration: rendered.duration, format: 'wav' }
  }
